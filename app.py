...
if "_clear_fields" not in st.session_state:
    st.session_state._clear_fields = []
...
# Clear input fields from last run
for key in st.session_state._clear_fields:
    st.session_state[key] = ""
st.session_state._clear_fields.clear()
...
if onglet_actif == "STOCK":
    ...
    if sous == "Matière première":
            ...
            for ingr, val in st.session_state.stock_matieres.items():
                ...
                with col1:
                    st.markdown(f"**{ingr}** : {val:.3f} "
                                f"<span style='color:orange'>(dernier : {prec:.3f})</span>", unsafe_allow_html=True)
                add = st.text_input(f"Ajout {ingr}", key=f"add_{ingr}")
                adj = st.text_input(f"Ajust {ingr}", key=f"adj_{ingr}")
                actions.append((ingr, add, adj))

            if st.form_submit_button("Valider"):
                for ingr, add, adj in actions:
                    try:
                        if add:
                            v = float(add.replace(",", "."))
                            if v > 0:
                    ...
                    try:
                        ...
                        pass
                    st.session_state._clear_fields.append(f"add_{ingr}")
                    st.session_state._clear_fields.append(f"adj_{ingr}")
        ...
        for gamme, infos in st.session_state.produits_finis.items():
            ...
            with c1: st.markdown(bloc_html(gamme), unsafe_allow_html=True)
            with c2: st.markdown(bloc_html(f"Unités : {total}"), unsafe_allow_html=True)
            with c3: st.markdown(bloc_html(f"Boîtes : {boites}"), unsafe_allow_html=True)
            with c4:
